...
export function ResultInterpretation({ result }: { result: TaskResult }) {
  const items = [
    ...
    `声纹层面：相似度从 ${result.speaker.simBefore.toFixed(3)} 降至 ${result.speaker.simAfter.toFixed(3)}，已有效破坏可克隆性。`,
  ...
  return (
    <EvidenceCard title="结果解读（自动生成）">
      <div className="grid gap-3 md:grid-cols-2">
        {items.map((item) => (
          <p key={item} className="rounded-xl border border-white/10 bg-slate-950/45 p-4 text-sm leading-6 text-slate-200">
            {item}
          </p>
        ))}
      </div>
