...
export function formatSimilarityDelta(delta: number | null | undefined) {
  ...
  if (value < 0) return `下降 ${Math.abs(value).toFixed(3)}`
  if (value > 0) return `上升 ${value.toFixed(3)}`
  return '无明显变化'
}
...
export function formatEmbeddingDistanceDelta(delta: number | null | undefined) {
  const value = optionalMetricNumber(delta)
  if (value === null) return '未生成'
  if (value > 0) return `增加 ${value.toFixed(3)}`
  if (value < 0) return `减少 ${Math.abs(value).toFixed(3)}`
  return '无明显变化'
}
...
export function generateCloneMetricInsights(input: CloneMetricInput) {
  ...
  if (similarityDropAbs !== null) {
    ...
    const magnitude = similarityDropAbs === 0 ? '' : ` ${Math.abs(similarityDropAbs).toFixed(3)}`
    items.push(
  ...
  if (embeddingDistanceDelta !== null) {
    ...
    const magnitude = embeddingDistanceDelta === 0 ? '' : `，绝对${direction} ${Math.abs(embeddingDistanceDelta).toFixed(3)}`
    items.push(
