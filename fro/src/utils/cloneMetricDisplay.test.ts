...
assert(display.similarityBefore === '0.449', 'Speaker Similarity before should keep three decimals')
assert(display.similarityAfter === '-0.042', 'Negative protected similarity must not be clamped')
assert(display.similarityDeltaText === '下降 0.491', 'Speaker Similarity delta should be an absolute decrease')
assert(display.embeddingDistanceBefore === '0.551', 'Embedding distance before should keep three decimals')
assert(display.embeddingDistanceAfter === '1.042', 'Embedding distance above 1 must not be clamped')
assert(display.embeddingDistanceDeltaText === '增加 0.491', 'Embedding distance delta should be an absolute increase')
assert(display.similarityDropAbsText === '0.491', 'Similarity drop amount should be absolute, not percent')
