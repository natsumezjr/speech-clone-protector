...
class SemanticEncoderEnsemble:
    ...
    def _extract_whisper(self, wave_16k):
        encoded_chunks = []
        valid_encoder_frames = 0
        max_samples = int(self.whisper_mel.n_samples)
        for wave_chunk in wave_16k.split(max_samples, dim=-1):
            actual_frames = min(
                wave_chunk.shape[-1] // self.whisper_mel.hop_length,
                self.whisper_mel.max_frames,
            )
            input_features = self.whisper_mel(wave_chunk)
            if self.whisper_backend == "openai":
                hidden = self.encoder_whisper(input_features)
            else:
                hidden = self.encoder_whisper(input_features=input_features).last_hidden_state
            valid_frames = min(hidden.shape[1], max(1, actual_frames // 2))
            encoded_chunks.append(hidden[:, :valid_frames, :])
            valid_encoder_frames += valid_frames

        self.whisper_valid_frames = valid_encoder_frames * 2
    ...
    def _extract_s3(self, wave_16k):
        ...
        positional_frames = int(self.encoder_s3.positional_embedding.shape[0])
        encoder_stride = int(getattr(self.encoder_s3, "stride", 1))
        max_mel_frames = positional_frames * encoder_stride * 2
        hidden_chunks = []
        for mel_chunk in mel.split(max_mel_frames, dim=-1):
            padded_mel, mel_len = s3tokenizer.padding([mel_chunk])
            hidden, _ = self.encoder_s3(
                padded_mel.to(self.device),
                mel_len.to(self.device),
            )
            hidden_chunks.append(hidden.transpose(2, 1))
